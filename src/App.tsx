...
function extractBody(source: string, folder: string) {
  ...
  body.querySelectorAll("a[href='#']").forEach((node) => {
    ...
    const title = anchor.getAttribute("title");
    const destination = title ? routeForTitle(title) : undefined;
    if (destination) anchor.setAttribute("data-route", destination.route);
  });
...
function routeForTitle(title: string) {
  ...
  if (normalized.includes("advice") || normalized.includes("metrics")) return screens[3];
...
function routeForIcon(icon: string) {
  ...
  if (icon === "monitoring" || icon === "auto_awesome") return screens[3];
...
function routeForText(text: string) {
  ...
  if (normalized.includes("schedule")) return screens[3];
  if (normalized.includes("settings")) return screens[2];
  return undefined;
}
